...
public class CuckooHashing {
    ...
    private int effectiveOperations; //NEW
    private Stack<Memo> st; //new
            ...
            for (int count = 0; count <= MAXTRIES; count++) {
                for (int i = 0; i < numHashFunctions; i++) {
                    ...
                    if (array[pos] == null) {
                        ...
                        currentSize++;
                        effectiveOperations++;
                        st.push(new Memo(pos, x, null, false));
                        return true;
                    }
                ...
                x = tmp;
                effectiveOperations++;
                st.push(new Memo(kick_pos, array[kick_pos], x, false));

            }
            st.push(new Memo(kick_pos, x, array[kick_pos], true));
            this.stash.add(x);
            return true;
        }
    }
    ...
    public void undo() {
        if (effectiveOperations != 0) {
            Memo memo = st.pop();
            if (memo.isInStash()) {
                stash.remove(memo.getValue());
                array[memo.getCurrentIndex()] = memo.getValue();
            } else
                array[memo.getCurrentIndex()] = memo.getPreValue();
            effectiveOperations--;
        }
